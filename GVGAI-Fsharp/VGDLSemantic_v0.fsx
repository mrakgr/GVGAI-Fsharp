...
let runSematic gameDesc =
    ...
    let id_map, tree_map =
        ...
        let rec flattenSpriteSet (spriteset: _ list) =
            ...
            let rec loop map parent_attrs (tree: _ list) =
                match tree with
                | h::hy::hs ->
                    let name,attrs_child = 
                        get_sprite_attrs h
                        |>  fun (name, attrs_child) -> //Check that there are no duplicates in attrs_child
                            if attrs_child.Length <> (List.distinctBy distinction_func attrs_child).Length then
                                failwithf "%s has duplicate attribute entries!" name
                            name, attrs_child
                    let attrs = 
                        List.append attrs_child parent_attrs |> List.distinctBy distinction_func
                    let new_map = map_add_if map (name,attrs)
                    match hy with
                    | SubSprites x -> 
                        let new_map' = loop new_map attrs x
    ...
    id_map, tree_map, level_mapping_set, tagged_eos, tagged_neos, tree_rec.termination_set, reverse_hierarchy, initializer_map