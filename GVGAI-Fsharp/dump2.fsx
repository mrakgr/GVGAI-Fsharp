...
type VGDLSprite =
    ...
    val queue : ResizeArray<InteractionTypes>
...
type VGDLGame(gameDesc: string, levelDesc: string, global_speed: float32, outcome_ref) as this =
    ...
    let binary_effect_manager_process =
        ...
        let inline interaction_function messages i (ra: VGDLSprite) (rb: VGDLSprite option) = // This one sensds messages to the queue
            for x in messages do
                match x with
                | KillSprite -> killsprite_manager_add i
                | ScoreChange x -> score <- score+x; score_string <- sprintf "Score: %i" score
                | TurnAround | StepBack -> ra.queue.Add(x)
...
t.Split('\n') |> Array.map (fun x -> x.Split(':').[0].[4..].TrimEnd() |> fun x -> x+"="+x) |> Array.iter (printf "%s,")
